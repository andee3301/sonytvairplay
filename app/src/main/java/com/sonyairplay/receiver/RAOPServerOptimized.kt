...
/**
 * RAOPServerOptimized
 * - Uses NativeAlac.decode when available for lower-latency in-memory decoding
 * - Falls back to FFmpegAlacDecoder when native decode not available
 * - RTCP support via RtcpManager
 */
object RAOPServerOptimized {
    private const val TAG = "RAOPServerOpt"
                    ...
                    synchronized(lock) {
                        ...
                        if (cf != null && cf.exists() && cf.length() > ROTATE_SIZE_THRESHOLD) {
                            try {
                                ...
                                decodeExecutor.submit {
                                    try {
                                        var nativeDecoded = false
                                        if (NativeAlac.isAvailable()) {
                                            try {
                                                val data = rotated.readBytes()
                                                val pcmBytes = NativeAlac.decode(data)
                                                if (pcmBytes != null && pcmBytes.isNotEmpty()) {
                                                    val fos = FileOutputStream(pcmOut)
                                                    fos.write(pcmBytes)
                                                    fos.flush()
                                                    fos.close()
                                                    AudioPlayer.playPcmFile(pcmOut.absolutePath)
                                                    rotated.delete()
                                                    File(pcmOut.absolutePath).delete()
                                                    nativeDecoded = true
                                                }
                                            } catch (e: Exception) {
                                                Log.w(TAG, "Native decode failed, falling back", e)
                                            }
                                        }
